...
def get_user_bot_config(user_id: str) -> dict | None:
    ...
    try:
        ...
        _TIER_LIMITS = {
            ...
            "pro":   {"comments": 400,"bloggers": 200, "replies": 200, "neighbor": True},
        }
        ...
        reply_persona_prompt = load_persona_prompt(user_id, "reply")
...
def add_pending_comment_sb(
    ...
    INSERT 전 (post_url, user_id) + status IN ('pending', 'approved', 'posted') 중복 체크.
    ...
    try:
        ...
        # 중복 사전 체크: 같은 게시물에 대기/승인/게시 상태가 이미 있으면 재삽입 방지
        # UNIQUE index 범위(pending, approved, posted)와 동일하게 맞춤
        existing = (
