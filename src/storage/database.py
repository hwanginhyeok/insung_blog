...
def get_recent_comments_for_blogger(blog_id: str, days: int = 30) -> list[str]:
    """
...
@_retry_on_db_lock(max_retries=3)
def get_comment_quality_stats(days: int = 7) -> dict:
    """
    댓글 품질 통계 조회
    - 성공률
    - 평균 길이
...
@_retry_on_db_lock(max_retries=3)
def get_low_quality_comments(days: int = 7, limit: int = 10) -> list[dict]:
...
@_retry_on_db_lock(max_retries=3)
def set_retry_decision(post_url: str, should_retry: bool) -> None:
    """재시도 여부 결정 (1: 예, 2: 아니오)"""
    with _conn() as conn:
        conn.execute(
            """
...
@_retry_on_db_lock(max_retries=3)
def get_retry_targets() -> list[dict]:
    """오늘 재시도할 대상 목록 조회"""
    with _conn() as conn:
        rows = conn.execute(
            """
...
@_retry_on_db_lock(max_retries=3)
def remove_from_retry_queue(post_url: str) -> None:
    """재시도 큐에서 제거 (성공 시)"""
    with _conn() as conn:
...
@_retry_on_db_lock(max_retries=3)
def record_blogger_reply(blog_id: str, reply_time_seconds: int, reply_text: str) -> None:
    """
    블로거의 답글 패턴 기록.
...
@_retry_on_db_lock(max_retries=3)
def update_blogger_pattern_score(blog_id: str, score: int) -> None:
    """패턴 분석 점수 업데이트 (0-100)"""
    with _conn() as conn:
        conn.execute(
            """
...
@_retry_on_db_lock(max_retries=3)
def get_blogger_suspicion_score(blog_id: str) -> int:
    """
...
@_retry_on_db_lock(max_retries=3)
def get_auto_blogger_list(min_score: int = 70) -> list[str]:
...
@_retry_on_db_lock(max_retries=3)
def should_skip_blogger(blog_id: str, threshold_high: int = 70, threshold_low: int = 40) -> tuple[bool, str]:
    """
    블로거 스킵 여부 결정
...
@_retry_on_db_lock(max_retries=3)
def get_blogger_behavior_stats(days: int = 30) -> list[dict]:
    """오토 블로거 추적 통계 조회"""
    with _conn() as conn:
        rows = conn.execute(
            """
