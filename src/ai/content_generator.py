...
def _analyze_images(client: Anthropic, image_paths: list[Path]) -> str:
    ...
    prompt = (
        ...
        "⚠️ 중요: 사진을 객관적으로 묘사하지 말고, 그 장면에서 느낄 수 있는 감정과 인상을 중심으로 써.\n"
        "'테이블 위에 흰 접시가 보인다' ❌ → '비주얼부터 먹음직스러워서 기대감 UP' ✅\n"
        "'붉은 조명이 설치되어 있다' ❌ → '분위기 있는 조명 덕에 데이트 느낌' ✅\n\n"
        "각 사진을 아래 형식으로 분석:\n"
        "사진 1: [장소 종류 | 핵심 인상/느낌 | 특별한 포인트]\n"
        "사진 2: [장소 종류 | 핵심 인상/느낌 | 특별한 포인트]\n"
        "...\n\n"
        "분석 항목:\n"
        "1. 장소 종류 (카페, 식당, 집, 야외 등)\n"
        "2. 음식이 있다면 식감/맛이 연상되는 인상 (먹음직스러운, 탱글탱글해 보이는 등)\n"
        "3. 전반적인 분위기와 느낌 (아늑한, 감성적인, 활기찬 등)\n"
        "4. 블로그 글감으로 쓸 수 있는 특별한 포인트"
    )
