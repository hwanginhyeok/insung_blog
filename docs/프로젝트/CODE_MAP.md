...
> 최종 갱신: 2026-03-22 (텔레그램 봇 리팩토링 — 멀티유저+이웃관리+알림 통합)
...
| `command_worker.py` | 명령 큐 워커 — Supabase bot_commands 10초 폴링, run/execute/retry/publish/**extract_blog_id** 핸들러, Semaphore(2), stale 명령 정리, **완료/실패 시 텔레그램 알림** | 2 | ✅ (알림 확장) |
| `telegram_bot.py` | 텔레그램 봇 통합 — **멀티유저(/start 등록)** + AI 초안 + 댓글 승인 + **이웃관리(/discover,/visit,/neighbor,/all)** + **큐 명령(/run,/execute,/retry)** + 설정 | 3 | ✅ (풀 리팩토링) |
| `telegram_bot_simple.py` | 텔레그램 봇 (레거시 — telegram_bot.py로 대체됨) | 3 | 📦 레거시 |
...
| `supabase_client.py` | Supabase 클라이언트 (service_role) — 생성 저장 + 봇 제어 평면 + `get_active_user_ids()`, `get_user_bot_config(user_id)`, **`get_user_by_chat_id()`, `register_chat_id()`, `get_chat_id_for_user()`** | ✅ (텔레그램 매핑 추가) |
...
| `telegram_notifier.py` | 텔레그램 알림 유틸 — 긴급 알림 + **`notify_command_result()`, `notify_command_failure()` 명령 결과 알림 (멀티유저 chat_id)** | ✅ (알림 확장)
