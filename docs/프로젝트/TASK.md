...
| # | 작업 | 중요도 | 상태 | 비고 |
|---|------|--------|------|------|
| COMMENT-HIST | 댓글 내역 + 개인 프롬프트 — DB 마이그레이션만 잔여 | P1 | 거의 완료 | 코드+배포 완료, SQL 실행만 남음 |
...
## BOT-UX: 봇 페이지 UX 전면 개선 — ✅ 완료 (2026-03-19)

> 실사용자 피드백 기반. 3단계 Stepper + STEP 분리 + 접기/펼치기 + 재실행 경고.

| # | 작업 | 상태 | 비고 |
|---|------|------|------|
| BU-01 | 3단계 플로우 Stepper (봇실행→승인→게시) | ✅ | 현재 단계 자동 하이라이트 |
| BU-02 | 봇 제어 카드 해체 → STEP 1/2/3 분리 | ✅ | 순서대로 배치 |
| BU-03 | STEP 3 게시 대기 목록 (approved fetch) | ✅ | 승인 후 UI에서 안 사라짐 |
| BU-04 | 봇 재실행 인라인 경고 | ✅ | pending/approved 존재 시 확인 |
| BU-05 | 쿠키/설정/이력 접기/펼치기 | ✅ | ChevronDown + 한줄 요약 |
| BU-06 | 실행 중 안내 + 경과시간 분:초 + 예상시간 | ✅ | "브라우저 닫아도 계속" |
| BU-07 | 상태 카드 대기 현황 (승인+게시) | ✅ | 2줄 표시 |
| BU-08 | REQUIREMENTS.md UX 요구사항 U1~U6 | ✅ | 섹션 7 |

---

## COMMENT-HIST: 댓글 내역 조회 + 개인 프롬프트 — 코드 완료, DB 마이그레이션 잔여 (2026-03-17)
...
| # | 작업 | 중요도 | 완료일 |
|---|------|--------|--------|
| BOT-UX | 봇 페이지 UX 전면 개선 (Stepper + STEP 분리 + 접기/펼치기) | P0 | 03-19 |
...
*마지막 업데이트: 2026-03-19 (BOT-UX 봇 페이지 UX 전면 개선)*
