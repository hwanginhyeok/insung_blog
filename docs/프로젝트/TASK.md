...
## 다음 작업 백로그 (2026-03-08 정리)

### P0: 버그 / 기술 부채

| # | 작업 | 상태 | 비고 |
|---|------|------|------|
| BUG-01 | AI 분석 시 formatting 항목 DB 저장 실패 근본 원인 수정 | 대기 | 수동 INSERT로 임시 해결. 새 유저 시 재발 |
| BUG-02 | 글 삭제 기능 (대시보드에서 삭제 버튼 + API) | 대기 | 테스트 글 정리 불가 |
| BUG-03 | 글 제목/본문 직접 편집 기능 | 대기 | 불러온 글 읽기 전용. AI 초안 미세 수정 불가 |
...
| # | 작업 | 상태 | 비고 |
|---|------|------|------|
| UX-06 | 사진 순서 드래그 변경 | 대기 | 업로드 순서 고정, 글 흐름 제어 불가 |
| UX-07 | 초안 직접 편집 (contentEditable 또는 textarea 전환) | 대기 | 단어 하나 바꾸려고 재생성 해야 함 |
| UX-08 | 카테고리별 프롬프트 커스텀 | 대기 | 맛집/카페/여행마다 다른 지시 가능 |
| UX-09 | 이미지 압축/리사이즈 (업로드 전 클라이언트 처리) | 대기 | Storage 용량 + Vision 토큰 절약 |

### P3: 확장 기능

| # | 작업 | 상태 | 비고 |
|---|------|------|------|
| EXT-01 | 글 히스토리 (버전 관리 — v1→v2→v3 비교/복원) | 대기 | — |
...
*마지막 업데이트: 2026-03-08 (다음 작업 백로그 정리)*
