...
| VISION-EMPATHY | 글쓰기 사진 묘사 공감 중심 변경 | P2 | ✅ 완료 | Vision 프롬프트 감성 중심 전환, PRODUCTION_SPEC 안티패턴 목록 추가 (2026-03-26) |
...
비교 검토한 알고리즘:
- **Weighted Fair Queuing**: 네트워크 대역폭 분배용, 유저 2~3명에 과설계
- **Work Stealing**: Go/Rust 런타임용, 워커 프로세스 풀에 적합하나 우리는 Semaphore 기반
- **Elastic Semaphore**: 가장 단순하고 현재 구조에 맞음

### 구현 범위

| # | 작업 | 상태 | 비고 |
|---|------|------|------|
| WE-01 | orchestrator.run()에서 블로거 방문을 N개씩 병렬화 | 대기 | asyncio.gather로 N명 동시 방문, N = 유저당_최대 |
| WE-02 | 유저별 슬롯 카운터 (활성 유저 추적) | 대기 | `_user_slot_count: dict[str, int]` 전역 관리 |
| WE-03 | 슬롯 양보 로직 (새 유저 등장 시 축소) | 대기 | 다음 블로거 방문부터 적용 (비선점, 실행중 태스크 유지) |
| WE-04 | 유저별 브라우저 컨텍스트 격리 | 대기 | 브라우저 인스턴스 공유 금지, 쿠키/세션 혼동 방지 |
| WE-05 | 유저별 DB 격리 검증 | 대기 | SQLite user_id별 분리, Supabase user_id 필터 재확인 |
| WE-06 | 슬롯 현황 로깅 + 텔레그램 알림 | 대기 | `[2/3 슬롯 사용중, A:2 B:1]` 형태 |

### 데이터 격리 주의사항

병렬 실행 시 유저 간 데이터가 섞이면 안 됨:

| 자원 | 격리 방식 | 현재 상태 |
|------|-----------|-----------|
| **Playwright 브라우저** | 유저별 별도 인스턴스 (쿠키/세션 분리) | ✅ 이미 분리됨 |
| **SQLite DB** | `data/{user_id[:8]}/comments.db` 유저별 파일 | ✅ 이미 분리됨 |
| **Supabase 쿼리** | 모든 쿼리에 `.eq("user_id", uid)` | ✅ 이미 적용됨 |
| **쿠키 파일** | `cookies/{user_id[:8]}_naver.json` 유저별 파일 | ✅ 이미 분리됨 |
| **orchestrator 상태** | 함수 로컬 변수 (전역 상태 없음) | ✅ 안전 |
| **Semaphore** | 전역 공유 (의도적) | ✅ 자원 제한 용도 |

### 확장 시나리오

| 유저 수 | RAM | 슬롯 | 유저당 | 예상 성능 |
|:---:|:---:|:---:|:---:|:---:|
| 1명 | 8GB | 3 | 3 | 블로거 50명 → ~1시간 |
| 2명 | 8GB | 3 | 1~2 | 각각 ~2시간 |
| 1명 | 32GB | 6 | 6 | 블로거 50명 → ~30분 |
| 3명 | 32GB | 6 | 2 | 각각 ~1.5시간 |
| 5명+ | 서버 | 10+ | 큐 우선순위 설계 | 별도 설계 필요 |
