...
| NAVER-SEARCH-API | 이웃 발견 소스를 네이버 검색 API로 전환 | P1 | 네이버 개발자 앱 등록 (NAVER_CLIENT_ID/SECRET) | 현재 Playwright search.naver.com 크롤링 → openapi.naver.com/v1/search/blog API 호출로 전환. 한 번에 100건, Playwright 불필요, 안정적. discoverer_v2의 _search_blogs 교체. 앱 등록 과정: developers.naver.com → 애플리케이션 등록 → 검색 API 체크 → WEB(localhost) → ID/Secret .env 추가 |
| NEIGHBOR-2HOP | 이웃의 이웃(2-hop) + 댓글 작성자 크롤 | P1 | NAVER-SEARCH-API | 좋은 이웃이 쌓인 후 고품질 소스로 활용. Playwright JS 로딩 대기 + iframe 접근 필요. 네이버 블로그 동적 로딩 때문에 단순 HTML 파싱 불가 — scroll+wait+frame 전환 구현. 1명당 5~10초로 느리지만 적합도 높음. 인기 블로그 댓글 작성자(3회+ = 소통형) 추출도 포함 |
